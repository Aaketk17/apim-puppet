...
class apim_common::params {
  ...
  case $profile {
    ...
    default: {
      $pack = "wso2am-${version}"
      $remote_pack = "https://github.com/wso2/product-apim/releases/download/v4.4.0/wso2am-4.4.0.zip"
      $server_script_path = "${product_dir}/${pack}/bin/api-manager.sh"
      $pid_file_path = "${product_dir}/${pack}/wso2carbon.pid"
      $optimize_params = ""
    }
  ...
  $wso2am_db_url = 'jdbc:mysql://apim-db-instance.cgk7myovdx4l.ap-south-1.rds.amazonaws.com:3306/apim_db?verifyServerCertificate=false&amp;useSSL=false&amp;allowPublicKeyRetrieval=true'
  $wso2am_db_username = 'apimadmin'
  $wso2am_db_password = 'kj#$r435%7df'
  $wso2am_db_type = 'mysql'
  $wso2am_db_validation_query = 'SELECT 1'

  $wso2shared_db_url = 'jdbc:mysql://apim-db-instance.cgk7myovdx4l.ap-south-1.rds.amazonaws.com:3306/shared_db?verifyServerCertificate=false&amp;useSSL=false&amp;allowPublicKeyRetrieval=true'
